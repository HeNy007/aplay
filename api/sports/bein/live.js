function FUCKUTHIEF(){
document.write('<ul id="matchKey">');


document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=1\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/1.jpg">');
document.write('    <span>Live 🔴 1</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=2\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/2.jpg">');
document.write('    <span>Live 🔴 2</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=3\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/3.jpg">');
document.write('    <span>Live 🔴 3</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=4\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/4.jpg">');
document.write('    <span>Live 🔴 4</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=5\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/5.jpg">');
document.write('    <span>Live 🔴 5</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=6\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/6.jpg">');
document.write('    <span>Live 🔴 6</span>');
document.write('  </li>');
document.write('  <li class="menu2" tabindex="0" onclick="window.location.href=\'https://aliwixplay.tn/player/qaliwix?src=7.jpg\'">');
document.write('    <img title="Aliwix SP" src="https://aliwixplay.tn/api/sports/bein/img/7.jpg">');
document.write('    <span>Live 🔴 7</span>');
document.write('  </li>');










document.write('</ul>');
} 
